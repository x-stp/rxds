...
.PHONY: all build debug clean test test-coverage lint-install lint lint-fix security fmt vet tidy check ci help
...
debug:
	@echo "building $(BINARY_NAME) (debug, perf-friendly)..."
...
help:
	@echo "targets:"
	@echo "  build          build the binary (stripped)"
	@echo "  debug          build with full DWARF + no inlining (for perf/pprof)"
	@echo "  test           run tests"
