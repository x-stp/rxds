...
func macSHA1(key []byte) hash.Hash {
	// Scanner note: SHA-1 is only used for legacy TLS configurations.
	// The standard library sha1 does not expose a ConstantTimeSum API, so we
	// avoid a constant-time wrapper here.
...
// AEAD interface is used by both conn.go and cipher_suites.go
// conn.go uses it, but it's defined here.
type aead interface {
	cipher.AEAD
...
// Note: zcrypto/upstream sometimes wrap SHA-1 to force constant-time HMAC sums.
// We intentionally don't do that here because the standard library sha1.New()
// does not implement ConstantTimeSum, and scanning doesn't benefit from it.
