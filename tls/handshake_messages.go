...
// For scanning we only currently care about ALPN negotiation.
...
// TLS 1.3 CertificateRequest. RFC 8446 Section 4.3.2.
...
func (m *certificateRequestMsgTLS13) marshal() []byte {
	...
	b.AddUint24LengthPrefixed(func(b *cryptobyte.Builder) {
		...
		b.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
			...
			if m.scts {
				// RFC 8446 Section 4.4.2.1: client Certificate extensions MUST correspond
				// to extensions in the server's CertificateRequest.
				b.AddUint16(extensionSCT)
				b.AddUint16(0) // empty extension_data
			}
			if len(m.supportedSignatureAlgorithms) > 0 {
...
func (m *newSessionTicketMsg) marshal() (x []byte) {
	...
	x[2] = uint8(length >> 8)
	x[3] = uint8(length)
	x[4] = uint8(m.lifetimeHint >> 24)
	x[5] = uint8(m.lifetimeHint >> 16)
	x[6] = uint8(m.lifetimeHint >> 8)
	x[7] = uint8(m.lifetimeHint)
